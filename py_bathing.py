...
class py_bathing:
    ...
    def _from_analyze(self, from_stmt):
        ...
        if type(from_stmt) is str:
            ...
            join_type = None
            for jt in ["left join", "inner join", "right join"]:
                if jt in from_stmt.keys():
                    join_type = jt
                    break
            if join_type:
                join_conditions = None
                print(from_stmt[join_type])
                if 'on' in from_stmt[join_type]:
                    join_conditions = [["{}.{}".format(*cond.split('.')) for cond in from_stmt[join_type]['on']['eq']]]
                self.from_ans += "{}({}, {}, \"{}\").".format(
                    join_type,
                    from_stmt[join_type]['value'] + " " + from_stmt[join_type]['name'],
                    join_conditions,
                    join_type.split()[0]
                )
